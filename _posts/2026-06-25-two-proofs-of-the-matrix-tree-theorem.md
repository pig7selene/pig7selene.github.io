...
tags: [graph-theory, matrix-tree-theorem, laplacian, determinant]
math: true
---
...
We prove Kirchhoff's Matrix-Tree Theorem by induction on both the number of vertices and the number of edges of the graph $G$.
...
Otherwise, we may assume that there exists an edge $e=(i,j)$ adjacent to $i$. For any spanning tree $T$, either $e \in T$ or $e \notin T$, so
...
Since $L_G=BB^T$, this also proves that $L_G$ is positive semidefinite.
